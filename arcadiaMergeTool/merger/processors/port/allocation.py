...
@process.register
def _(
    x: mm.information.PortAllocation,
    ...
    dest:
        Destination model to add Port Allocations to
    src:
        Source model to take Port Allocations from
    base:
        Base model to check Port Allocations against
    mapping:
    ...
    if (isinstance(destParent, mm.fa.FunctionInputPort)
        ...
        LOGGER.fatal(
            f"[{process.__qualname__}] Port Allocation parent is not a valid parent, Port uuid [%s], class [%s], parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.uuid,
            x.__class__,
            destParent.name,
            destParent.uuid,
    ...
    if (len(matchingPort) > 0):
        ...
        LOGGER.debug(
            f"[{process.__qualname__}] Create new Port Allocation uuid [%s], parent name [%s], uuid [%s], class [%s], dest parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.uuid,
