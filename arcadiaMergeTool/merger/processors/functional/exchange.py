...
def __findMatchingExchange(targetCollection, srcExch, 
                           ...
                           mappedSourcFuncPortEntry: MergerElementMappingEntry | None, 
                           mappedTargetFuncPortEntry: MergerElementMappingEntry | None, 
                           mapping: MergerElementMappingMap) -> m._obj.ModelElement | bool:
    ...
    # mappedSourceComp =  mappedSourcFuncEntry[0] if mappedSourcFuncEntry is not None else None
    # mappedTargetComp =  mappedTargetFuncEntry[0] if mappedTargetFuncEntry is not None else None

    # srcExchSourceComp = mapping.get((srcExch.source.parent._model.uuid, srcExch.source.parent.uuid))
    # srcExchTargetComp = mapping.get((srcExch.target.parent._model.uuid, srcExch.target.parent.uuid))

    for tgtExch in targetCollection:
        # tgtExchSourceComp = mapping.get((tgtExch.source.parent._model.uuid, tgtExch.source.parent.uuid))
        # tgtExchTargetComp = mapping.get((tgtExch.target.parent._model.uuid, tgtExch.target.parent.uuid))
        ...
        if tgtExch.name == srcExch.name:
            ...
            # if srcExch.name == "Confirmation canceling":
            #     print(">>>>", tgtExch)
            #     print(">>>>", tgtExch.source)
            #     print(">>>>", tgtExch.target)
            #     print("====", srcExch)
            #     print("====", srcExch.source)
            #     print("====", srcExch.source.exchanges)
            #     print("====", srcExch.target)
            #     print("====", srcExch.target.exchanges)
            #     print("====", srcExch.source.parent.name)
            #     print("====", srcExch.target.parent.name)
            #     print("????", mappedSourcFuncEntry)
            #     print("????", mappedTargetFuncEntry)
            ...
            tgtExchMappedSource = mapping.get((tgtExch._model.uuid, tgtExch.source.uuid))
            tgtExchMappedTarget = mapping.get((tgtExch._model.uuid, tgtExch.target.uuid))
            ...
            if tgtExchMappedSourceFunc is not None and tgtExchMappedTargetFunc is not None and tgtExchMappedSourceFunc == mappedSourcFuncEntry and tgtExchMappedTargetFunc == mappedTargetFuncEntry:
                ...
                return tgtExch
            # else:
            #     # postpone exchange processing if exists one without the ports attached
            #     return True
...
def _(
    ...
    sourceFunctionPortMap = mapping.get((x._model.uuid, x.source.uuid)) # pyright: ignore[reportOptionalMemberAccess] expect source exists in this context
    targetFunctionPortMap = mapping.get((x._model.uuid, x.target.uuid)) # pyright: ignore[reportOptionalMemberAccess] expect target exists in this context
    ...
    ex = __findMatchingExchange(targetCollection, x, sourceFunctionMap, targetFunctionMap, sourceFunctionPortMap, targetFunctionPortMap, mapping)
