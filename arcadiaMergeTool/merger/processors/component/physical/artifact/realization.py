...
@process.register
def _(
    x: mm.epbs.PhysicalArtifactRealization,
    ...
    ==========
    x:
        Physical Artifact Realization to process
    dest:
        Destination model to add Physical Artifact Realizations to
    src:
        Source model to take Physical Artifact Realizations from
    base:
        Base model to check Physical Artifact Realizations against
    mapping:
    ...
    if (isinstance(destParent, mm.epbs.ConfigurationItem)
        ...
        LOGGER.fatal(
            f"[{process.__qualname__}] Physical Artifact Realization parent is not a valid parent, Function uuid [%s], class [%s], parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.uuid,
            x.__class__,
            destParent.name,
            destParent.uuid,
    ...
    if (len(matchingFunction) > 0):
        ...
        LOGGER.debug(
            f"[{process.__qualname__}] Create new Physical Artifact Realization uuid [%s], parent name [%s], uuid [%s], class [%s], dest parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.uuid,
