...
@process.register
def _(
    x: mm.cs.PhysicalLinkCategory,
    dest: CapellaMergeModel,
    src: CapellaMergeModel,
    base: CapellaMergeModel,
    mapping: MergerElementMappingMap,
) -> bool:
    """Find and merge Physical Link Categories

    Parameters
    ==========
    x:
        Physical Link Category to process
    dest:
        Destination model to add Physical Link Categories to
    src:
        Source model to take Physical Link Categories from
    base:
        Base model to check Physical Link Categories against
    mapping:
    ...
    if (isinstance(destParent, mm.pa.PhysicalComponent)
        ...
        targetCollection = destParent.physical_link_categories # pyright: ignore[reportAttributeAccessIssue] expect ports are already there
    else:
        LOGGER.fatal(
            f"[{process.__qualname__}] Physical Link Category parent is not a valid parent, Physical Link Category name [%s], uuid [%s], class [%s], parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.name,
            x.uuid,
            x.__class__,
            destParent.name,
    ...
    if (len(matchingCapability) > 0):
        ...
        LOGGER.debug(
            f"[{process.__qualname__}] Create new Physical Link Category name [%s], uuid [%s], parent name [%s], uuid [%s], class [%s], dest parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.uuid,
