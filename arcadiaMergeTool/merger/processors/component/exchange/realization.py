...
@process.register
def _(
    x: mm.fa.ComponentExchangeRealization,
    ...
    if (len(matchingComponentExchange) > 0):
        ...
        LOGGER.debug(
            f"[{process.__qualname__}] Create new Component Exchange Realization uuid [%s], parent name [%s], uuid [%s], class [%s], dest parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.uuid,
