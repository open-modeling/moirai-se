...
@process.register
def _(
    x: mm.information.ExchangeItemElement,
    ...
    ==========
    x:
        Exchange Item Element to process
    dest:
        Destination model to add Exchange Item Elements to
    src:
        Source model to take Exchange Item Elements from
    base:
        Base model to check Exchange Item Elements against
    mapping:
    ...
    if isinstance(destParent, mm.information.ExchangeItem):
        ...
        LOGGER.fatal(
            f"[{process.__qualname__}] Exchange Item Element parent is not a valid parent, Exchange Item Element name [%s], uuid [%s], class [%s], parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.name,
            x.uuid,
            x.__class__,
            destParent.name,
    ...
    if (len(matchingExchangeItem) > 0):
        ...
        LOGGER.debug(
            f"[{process.__qualname__}] Create new Exchange Item Element name [%s], uuid [%s], parent name [%s], uuid [%s], class [%s], dest parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.name,
            x.uuid,
