...
def _(
    ...
    """Find and merge Boolean Type

    Parameters
    ==========
    x:
        Boolean Type to process
    dest:
        Destination model to add Boolean Type to
    src:
        Source model to take Boolean Type from
    base:
        Base model to check Boolean Type against
    mapping:
    ...
    if (isinstance(destParent, inf.DataPkg)
        ...
        LOGGER.fatal(
            f"[{process.__qualname__}] Boolean Type parent is not a valid parent, Boolean Type name [%s], uuid [%s], class [%s], parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.name,
            x.uuid,
            x.__class__,
            destParent.name,
    ...
    if (len(matchingPropertyValuePackages) > 0):
        ...
        LOGGER.debug(
            f"[{process.__qualname__}] Create new Boolean Type name [%s], uuid [%s], parent name [%s], uuid [%s], class [%s], dest parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.name,
            x.uuid,
