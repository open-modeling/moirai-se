...
def _(
    ...
    LOGGER.debug(
        f"[{process.__qualname__}] processing logical function package [%s], class [%s], uuid [%s], model name [%s], uuid [%s]",
        x.name,
        x.__class__,
        x.uuid,
        x._model.name,
    ...
    if mapping.get((x._model.uuid, x.uuid)) is None:
        mapping[(x._model.uuid, x.uuid)] = (dest.model.la.function_pkg, False)
