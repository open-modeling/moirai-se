...
@process.register
def _(
    x: mm.capellacommon.StateMachine,
    ...
    ==========
    x:
        StateMachine to process
    dest:
        Destination model to add StateMachines to
    src:
        Source model to take StateMachines from
    base:
        Base model to check StateMachines against
    mapping:
    ...
    matchingStateMachine = list(filter(lambda y: y.name == x.name, targetCollection))

    if (len(matchingStateMachine) > 0):
        # assume it's same to take first, but theme might be more
        mapping[(x._model.uuid, x.uuid)] = (matchingStateMachine[0], False)
    else:
        LOGGER.debug(
            f"[{process.__qualname__}] Create new StateMachine name [%s], uuid [%s], parent name [%s], uuid [%s], class [%s], dest parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.name,
            x.uuid,
