...
def _(
    ...
    ==========
    x:
        Mission Package to process
    dest:
        Destination model to add Mission Packages to
    src:
        Source model to take Mission Packages from
    base:
        Base model to check Mission Packages against
    mapping:
    ...
    if isinstance(destParent, sa.SystemAnalysis):
        mapping[(x._model.uuid, x.uuid)] = (destParent.mission_pkg, False) # pyright: ignore[reportArgumentType] expect data package is there and valid
        return True
    elif isinstance(destParent, sa.MissionPkg):
    ...
    if (len(matchingElements) > 0):
        ...
        LOGGER.debug(
            f"[{process.__qualname__}] Create new Mission Packages name [%s], uuid [%s], parent name [%s], uuid [%s], class [%s], dest parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.name,
            x.uuid,
