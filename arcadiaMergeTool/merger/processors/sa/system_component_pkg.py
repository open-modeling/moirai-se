...
def _(
    ...
    ==========
    x:
        System Component Package to process
    dest:
        Destination model to add System Component Packages to
    src:
        Source model to take System Component Packages from
    base:
        Base model to check System Component Packages against
    mapping:
    ...
    if isinstance(destParent, sa.SystemAnalysis):
        ...
        return True
    elif isinstance(destParent, sa.SystemComponentPkg):
    ...
    if (len(matchingElements) > 0):
        ...
        LOGGER.debug(
            f"[{process.__qualname__}] Create new System Component Packages name [%s], uuid [%s], parent name [%s], uuid [%s], class [%s], dest parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.name,
            x.uuid,
