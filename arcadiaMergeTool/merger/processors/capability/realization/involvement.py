...
@process.register
def _(
    x: mm.capellacommon.CapabilityRealizationInvolvement,
    ...
    ==========
    x:
        Capability Realization Involvement to process
    dest:
        Destination model to add Capability Realization Involvement to
    src:
        Source model to take Capability Realization Involvement from
    base:
        Base model to check Capability Realization Involvement against
    mapping:
    ...
    if (isinstance(destParent, mm.la.CapabilityRealization)
        ...
        LOGGER.fatal(
            f"[{process.__qualname__}] Capability Realization Involvement parent is not a valid parent, Capability Realization Involvement uuid [%s], class [%s], parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.uuid,
            x.__class__,
            destParent.name,
            destParent.uuid,
    ...
    matchingCapabilityRealization = list(filter(lambda y: y.parent == mappedSource[0] and x.involved == mappedTarget[0], targetCollection)) # pyright: ignore[reportOptionalSubscript] check for none is above, mappedSource and mappedTarget are safe

    if (len(matchingCapabilityRealization) > 0):
        # assume it's same to take first, but theme might be more
        mapping[(x._model.uuid, x.uuid)] = (matchingCapabilityRealization[0], False)
    else:
        LOGGER.debug(
            f"[{process.__qualname__}] Create new Capability Realization Involvement uuid [%s], parent name [%s], uuid [%s], class [%s], dest parent name [%s], uuid [%s], class [%s], model name [%s], uuid [%s]",
            x.uuid,
